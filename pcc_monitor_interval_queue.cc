...
double MonitorInterval::ComputeDelayDevirance() const{
    if (received_packets_.empty() || received_packets_.front().sent_time ==
                                       received_packets_.back().sent_time) {
        return 0;
    }
  ...
  double rtt_dev = 0.0;
  double delay_avg = sum_delays / received_packets_.size();
  for (const ReceivedPacket& packet : received_packets_) {
    double delay = packet.delay.ToMicroseconds();
    double delta_rtt = delay - delay_avg;
    rtt_dev += delta_rtt * delta_rtt;
  }
  rtt_dev = sqrt(rtt_dev/received_packets_.size());
